...
## ---------------------------------------------------------------##
# Requirements:
# - PROJECT FOLDER: 	/opt/certs_distrib
# - SSH KEYPAIRS: 	id_rsa, id_rsa.pub
...
# Routers data
ROUTEROS_USER=mailman
ROUTEROS_SSH_PORT=22
ROUTEROS_PRIVATE_KEY=$DIR/id_rsa
ROUTEROS_PUBLIC_KEY=$DIR/id_rsa.pub
...
LOG=/var/log/letsencrypt/darouter_certs.log

# Cleaning LOG
clear
echo "" > $LOG

for ip in $(grep -v "#" $ROUTERS); do
  ...
  if [ ! $RESULT == 0 ]; then
    ...
    scp -P $ROUTEROS_SSH_PORT $ROUTEROS_PUBLIC_KEY "$ROUTEROS_USER"@"$ROUTEROS_HOST":"id_rsa.pub"
    if [echo $? == 0 ]; then
